...
> ✅ **Chỉ cần xong Module 1, bạn đã có một trợ lý AI riêng chạy offline trên máy — đó đã là một hệ thống hoàn chỉnh, dùng được ngay.**
...
**Chọn model theo RAM:** dưới 14GB RAM hoặc không có GPU → chọn **[1] qwen3:1.7b** (mặc định). Từ 16GB hoặc có GPU → có thể chọn [2]/[3] mạnh hơn.
</details>
...
#    đóng cửa sổ này (gõ exit) → mở PowerShell MỚI → cd lại thư mục → chạy lại .\0_setup\setup.ps1
...
> 💡 **`.venv` là gì?** Là "hộp" chứa riêng các thư viện Python của workshop, không ảnh hưởng máy bạn. Phải **bật (activate)** nó trước khi chạy lệnh `python`.

---

## 4. ⚠️ Luôn bật venv trước khi chạy `python`

Đây là lỗi phổ biến nhất. **Trước MỌI lệnh `python`**, hãy chắc venv đang bật (đầu dòng lệnh có chữ `(.venv)`):
...
```
**Track B — Notebook (khuyến nghị):** Notebook (`.ipynb`) là file mở **trong trình duyệt**, chạy từng ô code. Cách mở:
```bash
...
> ⚠️ **Nhớ bật venv trước mỗi lệnh `python`** (xem mục 4). Mỗi lệnh **in chữ ra màn hình**, mất ~10–30 giây trên CPU (nhanh hơn nếu có GPU); riêng `--build` của Module 2 lâu hơn chút vì phải tạo embedding cho tài liệu. Notebook và script cho **kết quả giống nhau** — notebook chạy trong trình duyệt, script chạy trong terminal.
...
Hỏi đáp trên tài liệu của bạn. **Phải `--build` index 1 lần trước:**
```bash
...
> Lưu ý: bảng thời lượng và nhãn "Agent chỉ demo" là **lịch dạy trên lớp**. Người **tự học** cứ làm đầy đủ cả 3 module.
...
| Giao diện | **Gradio** + **Open WebUI** | Gradio để tùy biến, Open WebUI có sẵn đầy đủ tính năng |
