...
2. Sửa rag_app.py: hàm load_documents() đọc cả .md và .pdf,
...
### 3. Thêm metadata filter cho RAG
...
### 4. Build app riêng dựa trên template

```
Tôi muốn build một app phân tích log bảo mật:
- Reuse module 1 và 2 của workshop
- Web UI để upload file log (.log, .txt) — dùng Open WebUI, hoặc tự viết bằng Gradio/FastAPI
- Phân tích: tìm anomaly, đối chiếu chính sách nội bộ, gợi ý hành động
...
- Đề cập file gốc nếu sửa (`Sửa hàm chunk_text trong rag_app.py`)
...
python rag_app.py --ask "câu test"
```
...
```
Hãy đọc README.md và rag_app.py,
